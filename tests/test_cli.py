"""CLI senza GUI: stesso motore, durata in secondi simulati e riepilogo."""
...
def test_default_run_lasts_600_seconds_and_is_conserved(capsys):
    ...
    assert "Seed 42 · 600.00 s simulati (12000 tick)" in out
    assert "Conservazione: OK" in out
...
def test_zero_duration_shows_missing_mean(capsys):
    ...
    out = capsys.readouterr().out
    assert "(0 tick)" in out
    assert "Tempo medio di percorrenza:" in out and "—" in out
...
def test_invalid_durations_are_rejected(duration, capsys):
    ...
    assert "durata" in capsys.readouterr().err
...
def test_failed_conservation_exits_with_error(monkeypatch, capsys):
    ...
    assert "Conservazione: VIOLATA" in capsys.readouterr().out
